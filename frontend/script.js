...
function prefered_contact(mode){
    ...
    console.log(Prefered_Mode_of_Contact)
    var userMessage = `
        <div class="user">
...
function handleDomainIdea(response) {
    ...
    if (last_response === "yes") {
        if (func > 0 && func < 4) {
            console.log(current_input!=="",ideadStep)
            if(current_input!==""){
        ...
        if (ideadStep === 0) {
            ...
            console.log(ideadStep,"came",func)
            ideadStep++;
            func++;
            ...
            botMessage = `
                <div class="left-part">
                    <div class="chat typing">
                        <i class='bx bxs-bot'></i>
                        <p>Please enter your Preferred Mode of Contact</p>
                    </div>
                    <div class="contact">
        ...
        <div class="chat typing">
            <i class='bx bxs-bot'></i>
            <p>No worries! If you have any questions, feel free to ask.</p>
    </div>`;
    setTimeout(function () {
...
function Description(){
    ...
    var messagesDiv = document.getElementById("chat-section");
    if (ideadStep <5) {ideadStep = 5;}
    console.log(ideadStep,func)
    if (ideadStep > 5) {
        var userInput = current_input;
        var userMessage = `
        <div class="user">
    ...
    setTimeout(function () {
        ...
        lead_Capturing(selectedDomain,username, email, contact, Prefered_Mode_of_Contact, projectDomain, projectDescription)
        emailvalid = false
        var botMessage2 = `
    <div class="left-part">
        <div class="chat typing">
...
async function handlespecificationClick(specification) {
    ...
    console.log(selectedDomain, specification, filteredData);
    ...
    var botMessage2 = `
        <div class="left-part">
            <div class="chat typing">
                <i class='bx bxs-bot'></i>
                <p>Having some idea want to share?</p>
            </div>
            <div class="inner-button">
...
async function handleDomainClick(domain) {
    ...
    let response = await fetch("httpss://lemobot2.netlify.app/Data_Fetch_Option_1");
    ...
    var botMessage = `
        <div class="left-part">
            <div class="chat">
                <i class='bx bxs-bot'></i>
                <p>Here are the specifications for ${domain}:</p>
            </div>
...
async function learnAboutProducts() {
    ...
    try {
        let response = await fetch("https://lemobot2.netlify.app/Data_Fetch_Option_1");
        if (response.ok) {
            let data = await response.json();

            console.log(getUniqueSpecifications(data));
...
//below fuctions are for getInTouch
var track = 0
var gete = 0;
function uploadFile() {
    ...
    if (userInput !== "") {
        ...
        if (step === 0) {
            ...
            <div class="left-part">
                <div class="chat typing">
                    <i class='bx bxs-bot'></i>
                    <p>Sorry, I'm just a demo chatbot!</p>
                </div>
                </div>`;
            setTimeout(function () {
                var typingMessage = messagesDiv.querySelector(".typing");
...
function handleIdeaResponse(response) {
    ...
    console.log(response, last_response);
    console.log(step);
    if (last_response === "yes") {
        if (step >=6 && step<7) {
            var userInput = current_input;
        ...
        if (step === 5) {
            console.log("handleIdeaResponse", step);
            botMessage = `
            <div class="left-part">
            <div class="chat typing">
            <i class='bx bxs-bot'></i>
            <p>Great! Please enter your idea Domain</p>
            </div>
            </div>`;
        } else if (step === 6) {
            ...
            console.log('File selected:', file_url);
            var botMessage1 = `
            <div class="left-part">
            <div class="chat typing">
                <i class='bx bxs-bot'></i>
                <p>Name: ${username}<br>Email: ${email}<br>Contact :${contact}<br>Mode of Contact: ${Prefered_Mode_of_Contact}<br>Project Domain: ${projectDomain}<br>projectDescription: ${file_url}</p>
            </div>
            </div>`;
        setTimeout(function () {
                messagesDiv.innerHTML += botMessage1;
            ...
            lead_Capturing("Get In Touch ",username, email, contact, Prefered_Mode_of_Contact, projectDomain, file_url)
            emailvalid = false
            var botMessage2 = `
            <div class="left-part">
                <div class="chat typing">
            ...
            step = 0;
            track = 0;
            console.log("end of function ",step)
        }else if (last_response === "no") {
        count = 0;
        step = 0;
        track = 0;
        lead_Capturing("Get In Touch",username, email, contact, Prefered_Mode_of_Contact, "None", "Non")
        emailvalid = false
        var botMessage1 = `
            <div class="left-part">
            <div class="chat typing">
            ...
            <div class="chat typing">
                <i class='bx bxs-bot'></i>
                <p>Thank You for Contacting Us. Our team will contact you shortly</p>
        </div>`;
        setTimeout(function () {
                messagesDiv.innerHTML += botMessage1;
                var typingMessage = messagesDiv.querySelector(".typing");
...
async function otherInquiries() {
    ...
    let response = await fetch("https://lemobot2.netlify.app/FAQs_Fetch");
